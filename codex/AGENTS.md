...
- If already inside a linked worktree, stay there and do not create another one without an explicit request.
- Prefer host-managed creation; from the shell, use only the gitignored `./.worktrees/<name>`. Manual project-external placement is forbidden for agents, skills, subagents, automations, and scripts, and is enforced by hooks.
- Remove with `git worktree remove` and `git branch -d`; force-discard only after user confirmation.
- Install dependencies per worktree instead of sharing or copying dependency directories.
