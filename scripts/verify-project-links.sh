...
check_tool() {
	...
	[[ -L "${base}/skills" ]] || {
		echo "Expected symlink: ${base}/skills" >&2
		exit 1
	}
	[[ -L "${base}/commands" ]] || {
		echo "Expected symlink: ${base}/commands" >&2
		exit 1
	}
	[[ -L "${base}/rules" ]] || {
		echo "Expected symlink: ${base}/rules" >&2
		exit 1
	}
	...
	[[ "${resolved_skills}" == "${AGENTS}/skills" ]] || {
		echo "${tool}: skills does not resolve to .agents/skills" >&2
		exit 1
	}
	[[ "${resolved_commands}" == "${AGENTS}/commands" ]] || {
		echo "${tool}: commands does not resolve to .agents/commands" >&2
		exit 1
	}
	[[ "${resolved_rules}" == "${AGENTS}/rules" ]] || {
		echo "${tool}: rules does not resolve to .agents/rules" >&2
		exit 1
	}
	...
	shopt -u nullglob
	[[ -n "${any_skill_md}" ]] || {
		echo "${tool}: no */SKILL.md found under ${base}/skills" >&2
		exit 1
	}
